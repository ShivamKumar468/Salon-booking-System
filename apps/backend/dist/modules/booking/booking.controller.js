...
const bookingCreateSchema = zod_1.z.object({
    ...
    stylistId: zod_1.z.string().uuid('Invalid stylist ID'),
    dateTime: zod_1.z.string().datetime('Invalid ISO date time string'),
});
...
async function ensureSeedData() {
    ...
    const stylistCount = await prisma_1.default.stylist.count();
    if (stylistCount === 0) {
...
router.get('/stylists', async (req, res) => {
    try {
        await ensureSeedData();
        const stylists = await prisma_1.default.stylist.findMany({
            include: {
                user: {
                    select: {
                        id: true,
...
// Cancel booking
router.post('/cancel/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
